...
## Descrtiption
GUI chat application that connects two communication partners directly over TCP using TLS.
...
The length of an RSA certificate can be of 8192 bits or even 16384 bits. 
The Edward Curve is not (yet?) available for usage on Windows OS.
...
The firewall has to allow the connection too.
...
The binary can be compiled to display the sha256.
Sha1 has been chosen because the thumbprint of a certificate in Windows also displays the sha1.
So it's more convenient to check.
...
You could also create your own root cert, install it trust it, exchange it, ...
